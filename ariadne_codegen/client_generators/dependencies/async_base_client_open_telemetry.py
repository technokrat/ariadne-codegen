...
class AsyncBaseClientOpenTelemetry:
    ...
    async def execute_incremental(
        ...
        **kwargs: Any,
        ...
        if self.tracer:
            generator = self._execute_incremental_with_telemetry(
                query=query,
                operation_name=operation_name,
                variables=variables,
                **kwargs,
    ...
    async def _execute_incremental_with_telemetry(
        self,
        query: str,
        operation_name: Optional[str] = None,
        variables: Optional[dict[str, Any]] = None,
        **kwargs: Any,
        ...
        with self.tracer.start_as_current_span(  # type: ignore
            ...
            if files and files_map:
                generator = self._execute_multipart_incremental_with_telemetry(
                    root_span=root_span,
                    query=query,
                    operation_name=operation_name,
                    variables=processed_variables,
                ...
                generator = self._execute_json_incremental_with_telemetry(
                    root_span=root_span,
                    query=query,
                    operation_name=operation_name,
                    variables=processed_variables,
    ...
    async def _execute_multipart_incremental_with_telemetry(
        self,
        root_span: Span,
        query: str,
        operation_name: Optional[str],
        ...
        **kwargs: Any,
        ...
        with self.tracer.start_as_current_span(  # type: ignore
            "incremental multipart request", context=set_span_in_context(root_span)
        ) as span:
            span.set_attribute("component", "GraphQL Client")
            span.set_attribute("query", query)
            span.set_attribute("operationName", operation_name or "")
            span.set_attribute(
                "variables", json.dumps(variables, default=to_jsonable_python)
            )
            span.set_attribute("map", json.dumps(files_map, default=to_jsonable_python))

            async for data in self._execute_multipart_incremental(
                query=query,
                operation_name=operation_name,
                variables=variables,
                files=files,
    ...
    async def _execute_json_incremental_with_telemetry(
        self,
        root_span: Span,
        query: str,
        operation_name: Optional[str],
        ...
        **kwargs: Any,
        ...
        with self.tracer.start_as_current_span(  # type: ignore
            "incremental json request", context=set_span_in_context(root_span)
        ) as span:
            span.set_attribute("component", "GraphQL Client")
            span.set_attribute("query", query)
            span.set_attribute("operationName", operation_name or "")
            ...
            async for data in self._execute_json_incremental(
                query=query,
                operation_name=operation_name,
                variables=variables,
                **kwargs,
